...
flags.DEFINE_string('weights', 'my_model.h5', 'path to save file')
...
def main(_argv):
    img = np.expand_dims(np.asarray(cv2.imread("test_image.jpg")),0)
    # np.expand_dims(np.asarray(cv2.imread(FLAGS.image)),0)
    M , N , C = img.shape[1:]
